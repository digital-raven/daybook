...
def convert_filter(module):
    """ Raises an exception if module is not a valid converter.

    KeyError if certain headings were missing.
    ValueError if convert_row was not a callable function.
    """
    d = module.__dict__
    ...
    if type(d['headings']) is not str:
        raise TypeError('"headings" needs to be a str.')
